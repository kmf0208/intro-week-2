...
runTest("will get the name and ages of each person in an array", function () {
  ...
  const namesAndAges = people.map(el => el.name +" - "+el.age);
...
runTest("can get the names of staff over 28", function () {
  ...
  const staffOver28 = staff.filter(staff => staff.age > 28);

  const namesOfStaffOver28 = staffOver28.map(staff => staff.name);
...
runTest("can find the index position of X", function () {
  ...
  const positionOfX = treasureMap.filter(el => el.indexOf("X"));
